...
class WaveguideDataset(Dataset):
    ...
    def __getitem__(self, idx):  # cond and params log normalized
        with h5py.File(self.h5_path, 'r') as f:
            ...
            # Stack into shape [4, 2] — row-wise (mode_i, weight_i)
            cond = torch.stack([mode_norm, weight_norm], dim=1)  # [4, 2]

        return cond.float(), params_norm.float(), pattern

    def denormalize_cond(self, cond):
        """
        Reverts normalized cond (shape [4,2]) back to original scale.
        """
        if cond.dim() == 2:
            # cond: [4, 2]
            mode_log = cond[:, 0] * \
            ...
            # cond: [B, 4, 2]
            mode_log = cond[:, :, 0] * torch.tensor(self.stdm).unsqueeze(
                0) + torch.tensor(self.meanm).unsqueeze(0)
            weight_log = cond[:, :, 1] * torch.tensor(self.stdw).unsqueeze(
                0) + torch.tensor(self.meanw).unsqueeze(0)

        mode = torch.expm1(mode_log)
        weight = torch.expm1(weight_log)
        return torch.stack([mode, weight], dim=-1)  # [4, 2] or [B, 4, 2]
